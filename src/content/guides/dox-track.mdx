...
**Dox Track closes the oldest gap in documentation: the feature shipped, and nobody updated the docs.** You register your product's source repositories, and when a **pull request merges** there, Dox distills it into a task and has the [docs agent](/guides/docs-agent) draft a documentation pull request — reviewed by you, never merged on its own.
...
`dox track test` fetches the latest PR merged into the tracked base branch, applies your path filters, and shows the docs task it would produce:

```bash
dox track test acme/api-server          # latest merged PR
dox track test acme/api-server --pr 128  # a specific PR
```
...
Run `dox track setup` for tailored instructions. There are three interchangeable access modes — all end in a `repository_dispatch` to your docs repo, whose agent workflow (from [`dox agent init`](/guides/docs-agent)) drafts the PR. Pick one:

### Mode A — Connect GitHub app (recommended for orgs & private repos)

The Netlify/Vercel-style path: from the admin dashboard, create + install **your own** Dox GitHub App in a couple of clicks, then choose which repos Track may watch — org-wide access to the repos you pick, with **no token to paste**.

Because Dox is self-hosted, there's no central Dox server holding a key — instead the **"Connect GitHub"** button uses GitHub's [App Manifest flow](https://docs.github.com/apps/sharing-github-apps/registering-a-github-app-from-a-manifest) to stand up an app that belongs entirely to you:

1. Set `DOX_AUTH_SECRET` on the deployed site (it encrypts the app's private key at rest) and `DOX_SITE_URL` (so GitHub redirects back correctly).
...
3. GitHub walks you through creating the app (permissions and the org webhook are pre-filled), then choosing repos. Dox captures the credentials automatically — the app carries its own webhook, so you don't set `DOX_TRACK_WEBHOOK_SECRET`.
...
2. On the deployed site, set `DOX_TRACK_WEBHOOK_SECRET=<secret>` and `DOX_GITHUB_TOKEN` (a fine-grained PAT that reads tracked PRs, dispatches to the docs repo)
...
`dox track setup` prints a ready `.github/workflows/dox-track.yml` for each tracked repo (with your base branch and path filters baked in). Commit it to the product repo and add a `DOX_DISPATCH_TOKEN` secret there with dispatch access to the docs repo.

## Preview docs before a feature ships

Sometimes you want the docs ready for review *before* the feature merges. Add the **`docs-preview`** label to an open PR (any access mode above) and Track drafts the documentation it will need as a separate PR — so reviewers see the docs alongside the code. The preview re-drafts on each push to the labelled PR, and the instruction tells the agent the PR is still in review and may change. (Commenting `@dox` on a PR does the same thing on demand.)

## Tracking your own docs repo

If you point Track at the repository that also holds your docs (e.g. Dox tracking Dox), the agent's own merged docs PRs would otherwise re-trigger it. Track guards against this automatically: it ignores any PR whose head branch starts with `dox/agent-` (the prefix the agent stamps on its branches). To keep it tidy, also exclude your docs from `paths` (e.g. don't list `src/content/**`) so doc-only PRs don't fan out.
...
Its token resolution is the same as the webhook's: an explicit token, else a connected GitHub App, else the `DOX_GITHUB_TOKEN` PAT chain.

## Environment variables

| Variable | Where | Purpose |
| --- | --- | --- |
| `DOX_AUTH_SECRET` | deployed docs site | Encrypts the connected GitHub App's private key at rest (Mode A) |
| `DOX_SITE_URL` | deployed docs site | Canonical site URL — GitHub redirects back here during Connect (Mode A) |
| `DOX_REPO_URL` | deployed docs site | The docs repo Track dispatches to; defaults to `siteConfig.repoUrl`. Set it when `site.ts` keeps the template default |
| `DOX_TRACK_WEBHOOK_SECRET` | deployed docs site | Enables `/api/track/webhook` for a manual webhook (Mode B); not needed when a GitHub App is connected |
| `DOX_GITHUB_TOKEN` | deployed docs site | Fine-grained PAT that reads tracked PRs + relays the dispatch (Mode B; also used by the Docs tasks queue) |
| `DOX_DISPATCH_TOKEN` | each tracked repo (Mode C) | Dispatch access to the docs repo |
| `ANTHROPIC_API_KEY` | docs repo secrets | Runs the agent inside the docs-repo workflow |

Mode A stores the app's credentials (id, installation id, private key, webhook secret) encrypted in admin settings — you don't set `DOX_GITHUB_APP_*` env vars unless you prefer to wire the app manually. The token resolver honors them if present: `DOX_GITHUB_APP_ID`, `DOX_GITHUB_APP_INSTALLATION_ID`, `DOX_GITHUB_APP_PRIVATE_KEY`.
