...
These tools query a **deployed** Dox site over HTTP — they use the same indexes and reports that power the live site, so an agent can reason about docs it didn't scaffold.
...
| Parameter | Required | Description |
|-----------|----------|-------------|
| `siteUrl` | Yes | Base URL of the deployed Dox site |
...
Fetches the Agent Readiness Score (0–100) for a deployed site — the same report as `/api/agent-readiness` and `dox check`, including per-signal subscores and the specific pages dragging the score down.
