...
By default, Dox shows a built-in "Page not found" screen when a URL doesn't match any content. You can replace it with your own branded 404 page written in MDX.
...
Dox automatically detects this file and renders it through the standard MDX pipeline — so all components (`<Card>`, `<Note>`, etc.) work normally.

## Fallback behavior

If `src/content/404.mdx` does not exist, Dox renders its built-in 404 UI. You can always delete or rename `404.mdx` to revert to the default.
