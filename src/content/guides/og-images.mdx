...
Dox generates dynamic OG images for every page so your docs look polished when shared on Twitter, Slack, Discord, and other platforms. Each image includes the page title, description, group label, and your site branding.
...
With zero configuration, Dox will:

- Use your **dark theme** brand palette for the image background, accent bar, and text colors
- Display the **site name** and **domain** (from `NEXT_PUBLIC_SITE_URL`) in the bottom bar
...
| `domain` | Domain shown in bottom bar | Hostname from `NEXT_PUBLIC_SITE_URL` |
...
Set `NEXT_PUBLIC_SITE_URL` in your hosting environment. Without it, crawlers resolve the relative `/api/og?...` URL against `localhost`, which is unreachable.
</Note>
...
In your hosting dashboard (Vercel, Netlify, etc.) or in `.env.local` for local testing:
