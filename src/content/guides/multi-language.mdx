...
description: Add first-class multi-language support to your Dox docs site with build-time translations, automatic fallbacks, and a built-in language switcher.
keywords: [i18n, internationalization, translation, multi-language, localization, locale]
---

Dox supports build-time multi-language documentation. Translated MDX files live in your repo alongside your primary content — fully version-controlled, statically generated, and indexed by search engines with real locale-specific URLs.
...
<Step title="Add an i18n block to docs.json">
...
```bash
npx create-dox translate --locale es --pages introduction,guides/getting-started
```
...
```bash
npx create-dox translate --locale es --pages guides/getting-started --force
```
...
Dox never returns a 404 for a missing translation. Instead, it serves the primary-language content with an informative banner:
...
```bash
npx create-dox translate --locale es --pages guides/getting-started
```
...
When an `i18n` config is present, Dox automatically adds `hreflang` and `canonical` link tags to every page:
...
The primary URL is always canonical, including on secondary-locale pages. Set `DOX_SITE_URL` so the correct domain appears in these tags:

```bash
DOX_SITE_URL=https://docs.example.com
```
...
If you use the Dox MCP server, you can translate docs directly from your AI assistant:
...
<Card title="CLI Reference" href="/guides/cli-reference" icon="wrench">
  Full reference for `translate` and all other create-dox commands.
</Card>

<Card title="MCP Server" href="/guides/mcp-server" icon="grid-round">
  Set up the Dox MCP server to translate docs from your AI assistant.
</Card>
